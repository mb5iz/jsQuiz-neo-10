...
**React 入門**です。  
ビルドツールなし（CDN だけ）で React を動かせるようにしてあります。  
**コンポーネント**と **props** の基本の問題となります。
jsQuiz09 でやった「分割代入」「テンプレートリテラルへの変数埋め込み」が、そのまま React の書き方につながっています。

## 課題内容

カフェのスタッフ紹介ページです。  
`Profile` コンポーネントに props（名前・役職・絵文字）を渡して、プロフィールカードを3枚表示します。
...
```html
<!-- ① import map：import に書く名前をCDNのURLに対応付ける表 -->
